...
-- Insert sample jobs with proper customer references
INSERT INTO public.jobs (customer_id, job_type, job_date, status, price, notes, vehicle_lock_details) VALUES
-- Use the first customer (John Smith) - need to get the actual customer_id
((SELECT id FROM customers WHERE name = 'John Smith'), 'car_lockout', '2024-01-20', 'paid', 85.00, 'Toyota Camry 2015, passenger door lock picked', 'Toyota Camry 2015 - front passenger door'),
((SELECT id FROM customers WHERE name = 'Sarah Johnson'), 'key_duplication', '2024-01-21', 'completed', 25.00, 'Made 2 copies of house key', NULL),
...
((SELECT id FROM customers WHERE name = 'Emily Chen'), 'lock_rekey', '2024-01-23', 'paid', 120.00, 'Rekeyed all locks after lost keys', 'Front door, back door, garage side door'),
((SELECT id FROM customers WHERE name = 'David Wilson'), 'car_key_programming', '2024-01-24', 'pending', 150.00, 'Program new Honda key fob', 'Honda Civic 2018 - remote start key'),
...
((SELECT id FROM customers WHERE name = 'Robert Taylor'), 'car_lockout', '2024-01-26', 'paid', 90.00, 'BMW 3 Series emergency lockout', 'BMW 3 Series 2019 - driver door'),
...
((SELECT id FROM customers WHERE name = 'Michael Davis'), 'key_duplication', '2024-01-28', 'completed', 180.00, 'Fleet vehicle keys - 12 copies made', 'Commercial van fleet keys'),
...
((SELECT id FROM customers WHERE name = 'John Smith'), 'key_duplication', '2023-12-15', 'paid', 30.00, 'House key backup copies', NULL),
...
((SELECT id FROM customers WHERE name = 'Emily Chen'), 'car_key_programming', '2023-10-10', 'paid', 125.00, 'Programmed replacement car key', 'Honda Accord 2016'),
((SELECT id FROM customers WHERE name = 'Mike Rodriguez'), 'lock_rekey', '2023-09-05', 'paid', 200.00, 'Office security upgrade', 'Main office and conference rooms'),
((SELECT id FROM customers WHERE name = 'Lisa Anderson'), 'car_lockout', '2024-01-30', 'pending', 95.00, 'Locked keys in trunk', 'Subaru Outback 2020');

-- Update some jobs to create variety in status and recent activity
UPDATE public.jobs SET 
  job_date = CURRENT_DATE,
  status = 'pending'
WHERE customer_id = (SELECT id FROM customers WHERE name = 'Amanda Miller');
