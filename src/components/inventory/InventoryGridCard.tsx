...
import { Edit, Trash2, KeyRound, SlidersHorizontal } from 'lucide-react';
...
export function InventoryGridCard({ item, showReorderNeed, onEdit, onAdjust, onDelete }: InventoryGridCardProps) {
    ...
    <Card className="h-full overflow-hidden border shadow-none transition-colors hover:border-primary/40">
      <CardContent className="p-5 space-y-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="flex-1 h-11 gap-2" onClick={() => onAdjust(item)}><SlidersHorizontal className="h-4 w-4" />Adjust</Button>
          <Button variant="ghost" className="h-11 w-11" aria-label={`Edit ${title}`} onClick={() => onEdit(item)}><Edit className="h-4 w-4" /></Button>
