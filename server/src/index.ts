...
app.get('/api/inventory-movements', authMiddleware, asyncRoute(async (req, res) => {
  const itemId = typeof req.query.itemId === 'string' ? req.query.itemId : undefined;
  res.json(await prisma.inventoryMovement.findMany({
    where: { userId: req.userId, ...(itemId ? { inventoryItemId: itemId } : {}) },
...
app.post('/api/inventory/:id/adjust', authMiddleware, mutation((tx, uid, req) => operations.adjustInventory(tx, uid, String(req.params.id), req.body), ['inventory:changed']));
