...
function requireEnv(name: string, minimumLength = 1): string {
  const value = process.env[name]?.trim();
  if (!value || value.length < minimumLength) {
    throw new Error(`${name} must be configured and contain at least ${minimumLength} characters`);
  }
  return value;
}
...
const CORS_ORIGIN =
  ...
  (isProduction ? requireEnv('CORS_ORIGIN') : 'http://localhost:5173');
const JWT_SECRET = requireEnv('JWT_SECRET', 32);
const OWNER_EMAIL = requireEnv('OWNER_EMAIL').toLowerCase();
