...
export class Operations {
  ...
  async saveInventory(tx: Prisma.TransactionClient, userId: string, value: unknown, id?: string) {
    ...
    if (before) {
      const updated = await tx.inventoryItem.update({ where: { id: before.id, userId, version: before.version }, data: { ...data, version: { increment: 1 } } });
      if (body.quantity !== undefined && updated.quantity !== before.quantity) {
        await tx.inventoryMovement.create({ data: {
          userId,
          inventoryItemId: before.id,
          type: 'correction',
          delta: updated.quantity - before.quantity,
  ...
  async adjustInventory(tx: Prisma.TransactionClient, userId: string, id: string, value: unknown) {
    const body = object(value);
    fields(body, ['delta', 'reason', 'note', 'expectedVersion']);
    const delta = body.delta;
    if (typeof delta !== 'number' || !Number.isSafeInteger(delta) || delta === 0 || Math.abs(delta) > 100000000) throw new ApiError(400, 'Quantity change must be a nonzero whole number');
    const reason = text(body.reason, 'Reason', true)!;
    if (!['purchased', 'used', 'damaged', 'returned', 'correction'].includes(reason)) throw new ApiError(400, 'Choose a valid adjustment reason');
    const note = text(body.note, 'Note');
    const item = await tx.inventoryItem.findFirst({ where: { id, userId } });
    if (!item) throw new ApiError(404, 'Item not found');
    if (body.expectedVersion !== item.version) throw new ApiError(409, 'Stock changed. Refresh before adjusting it.');
    const quantityAfter = item.quantity + delta;
    if (quantityAfter < 0) throw new ApiError(409, `Only ${item.quantity} units are available`);
    const updated = await tx.inventoryItem.update({
      where: { id: item.id, userId, version: item.version },
  ...
  async bulkInventory(tx: Prisma.TransactionClient, userId: string, value: unknown) {
    ...
    for (const item of items) {
      ...
      if (next !== item.quantity) await tx.inventoryMovement.create({ data: {
        userId,
        inventoryItemId: item.id,
        type: 'correction',
  ...
  async saveJob(tx: Prisma.TransactionClient, userId: string, value: unknown, id?: string) {
    ...
    const jobId = before?.id || randomUUID();
    await this.adjustStock(tx, userId, deltas, before?.id);
    ...
    return tx.job.create({ data: { ...data, id: jobId, jobType: data.jobType as string, userId, stockTracked: true, inventory: { create: links } } as Prisma.JobUncheckedCreateInput, include: jobInclude });
  }

  private async adjustStock(tx: Prisma.TransactionClient, userId: string, deltas: Map<string, number>, jobId?: string) {
    for (const [id, delta] of [...deltas].sort(([a], [b]) => a.localeCompare(b))) {
      if (!delta) continue;
      const before = await tx.inventoryItem.findFirst({ where: { id, userId } });
      if (!before) throw new ApiError(409, 'An inventory item is unavailable. No changes were saved.');
      ...
      await tx.inventoryMovement.create({ data: {
        ...
        inventoryItemId: id,
        jobId,
        type: delta > 0 ? 'job_used' : 'job_restored',
        delta: -delta,
        quantityBefore: before.quantity,
        quantityAfter: before.quantity - delta,
        reason: delta > 0 ? 'Used on completed job' : 'Restored from reopened or removed job',
      } });
    }
  }
  ...
  async deleteJob(tx: Prisma.TransactionClient, userId: string, id: string) {
    ...
    if (job.stockTracked && completed(job.status)) await this.adjustStock(tx, userId, new Map(job.inventory.map(link => [link.inventoryItemId, -link.quantityUsed])), job.id);
